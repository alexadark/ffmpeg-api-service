...
async function downloadYouTube(url, outputDir, options = {}) {
  ...
  try {
    ...
    const ytdlpOptions = {
      ...
      // Use multiple clients to bypass bot detection
      // Try web client first, then ios as fallback
    ...
    const infoOptions = {
      ...
      noWarnings: true,
      skipDownload: true,
      // Use same extractor args for info request
      extractorArgs: 'youtube:player_client=web,ios',
      userAgent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
    };
    ...
    if (stats.size === 0) {
      throw new Error('Downloaded file is empty - YouTube may be blocking the request. Try refreshing cookies or wait before retrying.');
    }

    console.log(`[YouTube] Download complete: ${outputFile} (${Math.round(stats.size / 1024 / 1024 * 100) / 100}MB)`);
    ...
    if (errorMessage.includes('empty')) {
      errorMessage = 'Download failed: YouTube returned empty content. This usually means: 1) Your cookies have expired (refresh them), 2) YouTube is rate-limiting the server IP, or 3) The video format is unavailable. Try again in a few minutes.';
    } else if (errorMessage.includes('Sign in') || errorMessage.includes('age')) {
      errorMessage = 'This video requires authentication. Please provide valid YouTube cookies.';
    } else if (errorMessage.includes('unavailable') || errorMessage.includes('private')) {
      errorMessage = 'Video is unavailable, private, or restricted in this region.';
    }
