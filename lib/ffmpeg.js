...
async function addSubtitles(inputPath, outputPath, subtitlesPath, options = {}) {
  ...
  try {
    ...
    // When using single quotes in the filter, we need to escape single quotes in the path
    // and convert Windows backslashes to forward slashes
    const escapedSubPath = subtitlesPath
      .replace(/\\/g, '/')     // Convert backslashes to forward slashes (Windows compatibility)
      .replace(/'/g, "'\\''"); // Escape single quotes by closing quote, escaping, and reopening
