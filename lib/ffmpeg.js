...
function buildXfadeCommand(files, outputPath, transitionDuration = 1, width = 1920, height = 1080, includeAudio = true) {
  ...
  if (includeAudio) {
    ...
    // 4. Faded in/out at transitions (afade)
    const audioFilters = [];
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      // Trim audio to its duration
      audioFilter += `,atrim=0:${duration}`;

      // Add delay to position audio at correct start time
      if (delayMs > 0) {
      ...
      if (i < files.length - 1) {
        const fadeOutStart = startOffset + duration - transitionDuration;
        audioFilter += `,afade=t=out:st=${fadeOutStart}:d=${transitionDuration}`;
      }

      // Add fade in at the start (except for first video)
      if (i > 0) {
        audioFilter += `,afade=t=in:st=${startOffset}:d=${transitionDuration}`;
      }
    ...
    filterComplex += `; ${audioInputs}amix=inputs=${files.length}:duration=longest:normalize=0[aout]`;
