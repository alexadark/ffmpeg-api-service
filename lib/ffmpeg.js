...
async function enhanceAudio(inputPath, outputPath, options = {}) {
  ...
  try {
    ...
    // 2. Advanced noise reduction using FFT denoiser
    // nf = noise floor, tn = track noise, tr = track residual, om = output mode
    audioFilters.push(`afftdn=nf=${noiseFloor}:tn=1:tr=1:om=o`);

    // 3. Noise gate: Remove background noise (keyboard, mouse clicks, room noise)
    // Opens gate at -35dB, closes at -45dB, smooth attack/release
    audioFilters.push('agate=threshold=-35dB:ratio=3:attack=10:release=100:makeup=0:knee=2');

    // 4. Voice boost (optional): Gentle presence boost BEFORE compression
    ...
    // 5. Gentle dynamic compression: Even out volume levels
    audioFilters.push('compand=attacks=0.3:decays=0.8:points=-80/-80|-45/-40|-27/-27|0/-20|20/-20');

    // 6. De-esser: Reduce harsh sibilance (reduce 6-8kHz)
    audioFilters.push('equalizer=f=7000:width_type=o:width=1:g=-2');

    // 7. Normalization to broadcast standard (-14 LUFS)
