...
function buildCinematicFilter(intensity = 1.0, adjustments = {}) {
  ...
  // Temperature adjustment: -50 to +50 maps to 4000K to 10000K
  ...
  const filters = [
    ...
    `colortemperature=temperature=${tempKelvin}`,
  ];
...
function buildCoolFilter(intensity = 1.0, adjustments = {}) {
  ...
  // Cool temperature: lower Kelvin (blue tint)
  const tempKelvin = 6500 + (temperature / 50) * 3500;
...
function buildWarmFilter(intensity = 1.0, adjustments = {}) {
  ...
  // Warm temperature: higher Kelvin (orange tint)
  const tempKelvin = 6500 + (temperature / 50) * 3500;
...
function buildCustomFilter(intensity = 1.0, adjustments = {}) {
  ...
  // Temperature adjustment: convert to Kelvin range (4000K to 10000K)
  const tempKelvin = 6500 + (temperature / 50) * 3500;
